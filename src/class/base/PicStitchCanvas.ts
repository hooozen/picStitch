...
export default class PicStitchCanvas {
  ...
  draw() {
    if (this.ctx === null) return console.warn('init is required before drawing!')
    console.log('canvas image views: ', this.imageViews);
  ...
  _setWidth() {
    ...
    this.canvas.width = this.imageViews
      .reduce((width, view) => Math.min(width, view.clipView.width), Infinity);
    this.scaleRatio = this.canvas.width / this.canvas.offsetWidth;
    console.log('canvas ratio', this.canvas.width, this.canvas.offsetWidth)
    console.log('cavnas ratio', this.scaleRatio)
    console.log('canvas width:', this.canvas.width);
  }
  _setViewsWidth() {
    if (this.canvas === null) return console.warn('canvas not init!')
    this.imageViews.forEach(view => { view.setWidth(this.canvas!.width); });
  }
  _setViewsOffset() { }
  _setHeight() { }
  _scaleViews() {
